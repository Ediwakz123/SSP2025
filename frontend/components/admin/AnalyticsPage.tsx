...
import { getSeedDataStats, getAllAnalyses } from "../../lib/api-client";
...
export function AnalyticsPage({ accessToken }: AnalyticsPageProps) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [seedRes, analysesRes] = await Promise.all([
        getSeedDataStats(),
        getAllAnalyses(accessToken),
      ]);

      if (seedRes?.success) setSeedStats(seedRes.stats);
  ...
  const topCategory =
    seedStats &&
    Object.entries(seedStats.categories || {})
      .sort((a, b) => (b[1] as number) - (a[1] as number))[0]?.[0];

  const topZone =
    seedStats &&
    Object.entries(seedStats.zones || {})
      .sort((a, b) => (b[1] as number) - (a[1] as number))[0]?.[0];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Key metrics row (matches your screenshot style) */}
